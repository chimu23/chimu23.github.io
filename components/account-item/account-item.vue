<template>
	<view>
		<view @tap="tapAccount(account)" @longpress="longTapAccount(account)"
			:class="['item',account.isCollection?'collect':'']" hover-class="item-hover" v-for="(account,index) in accounts"
			:key="index">
			<view class="text">{{account.belong}}</view>
			<view class="value">{{account.account}}</view>
		</view>
		<account-card ref="accountCard" :account="currentAccount" @update:account="updateAccount"></account-card>
	</view>
</template>
